...
SoftwareSerial gsmSerial(3,2); // These pins are connected to GSM module( RX, TX )
...
{ if(GSM_operational())
{
gsmSerial.println("AT+COPS?"); //AT command for getting serivce provider name
gsmSerial.println();
}
...
delay(1000);
gsmSerial.print("GSM bot functonal"); // SMS BODY here in case u want change
// gsmSerial.print(i);
delay(500);
...
print_content(number);
gsmSerial.println("ATD"+ number +";"); // AT command for dialing up the number
gsmSerial.println();
on_call=true;
return(true);
}
...
if(GSM_operational())
gsmSerial.print("AT+CMGF=1"); // Setup in msging mode
delay(500);
 
if (GSM_operational())
{
gsmSerial.print("AT+CNMI=2,2,0,0,0\r" ); //Prints the msg on serial as soon as it arrives
delay(500);
...
if(key=='#')
{
gsmSerial.println("ATH"); //Termination action
gsmSerial.println();
Serial.println("Call Terminated");
print_head("Call Terminated");
delay(1000);
...
{
bool valid=false;
if(number.length()==14) // condition here
{valid=true;}
else
